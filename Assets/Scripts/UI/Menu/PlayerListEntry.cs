...
public class PlayerListEntry : MonoBehaviour {
    ...
    public void UpdateText() {
        ...
        switch (platform) {
            case "pc":
                platformImg.sprite = pcIcon;
                break;
            case "mobile":
                platformImg.sprite = phoneIcon;
                break;
        }
    ...
    string returnPlatform() {
        if (player.CustomProperties.ContainsKey("MobileSpecificProperty")) {
            return "mobile";
        }
        // If the custom property is not present, assume it's a PC
        return "pc";
    }
}