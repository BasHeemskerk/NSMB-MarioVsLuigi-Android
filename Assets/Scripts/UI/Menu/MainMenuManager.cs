...
public class MainMenuManager : MonoBehaviour, ILobbyCallbacks, IInRoomCallbacks, IOnEventCallback, IConnectionCallbacks, IMatchmakingCallbacks {
    ...
    public Toggle scrControlsToggle, ndsResolutionToggle, fullscreenToggle, livesEnabled, powerupsEnabled, timeEnabled, drawTimeupToggle, fireballToggle, vsyncToggle, privateToggle, privateToggleRoom, aspectToggle, spectateToggle, scoreboardToggle, filterToggle, globalServerToggle, vibrationToggle, useDpadToggle;
    public GameObject playersContent, playersPrefab, chatContent, chatPrefab;
    public TMP_InputField nicknameField, starsText, coinsText, livesField, timeField, lobbyJoinField, chatTextField;
    public Slider musicSlider, sfxSlider, masterSlider, lobbyPlayersSlider, changePlayersSlider, fpsSlider;
    ...
    public TMP_Text errorText, rebindCountdown, rebindText, reconnectText, updateText, fpsText, joinRoomTxt, lobbyServerText;
    ...
    public List<string> mobile_only_maps;
    ...
    private bool firstWelcomeWasSent = false;
    ...
    public void OnJoinedLobby() {
        ...
        if (!firstWelcomeWasSent) {
            LocalChatMessage("Welcome back to <color=red>M</color>vs<color=green>L</color>O!\nToday is " + System.DateTime.Now.ToString("dddd, MMMM dd, yyyy") + "! " + "\n<color=orange>GLHF!</color>");
            firstWelcomeWasSent = true;
        }
    }
    ...
    public void OnMasterClientSwitched(Player newMaster) {
        LocalChatMessage("MvsLO:\n" + newMaster.GetUniqueNickname() + " has become the Host", Color.green);
        ...
        if (newMaster.IsLocal) {
            ...
            LocalChatMessage("MvsLO:\n" + "You are the room's host! You can click on player names to control your room, or use chat commands. Do /help for more help.", Color.red);
        }
        UpdateSettingEnableStates();
    }
    public void OnJoinedRoom() {
        Debug.Log($"[PHOTON] Joined Room ({PhotonNetwork.CurrentRoom.Name})");
        LocalChatMessage("MvsLO:\n" + PhotonNetwork.LocalPlayer.GetUniqueNickname() + " joined the room", Color.red);
        EnterRoom();
    }
    ...
    public void OnPlayerEnteredRoom(Player newPlayer) {
        ...
        LocalChatMessage("MvsLO:\n" + newPlayer.GetUniqueNickname() + " joined the room", Color.red);
    ...
    public void OnPlayerLeftRoom(Player otherPlayer) {
        ...
        LocalChatMessage("MvsLO:\n" + otherPlayer.GetUniqueNickname() + " left the room", Color.red);
    ...
    public void ChangeDebugState(bool enabled) {
        ...
        if (!Settings.Instance.globalServer){
            levelDropdown.AddOptions(mobile_only_maps);
        }
    ...
    public void OnEvent(EventData e) {
        ...
        case (byte) Enums.NetEventIds.PlayerChatMessage: {
            ...
            message = sender.GetUniqueNickname() + ":\n" + message.Filter();
    ...
    private string[] fps_slider_values = new string[] { "30", "60", "120", "144" };
    ...
    private void LoadSettings(bool nickname) {
        ...
        fpsSlider.value = Settings.Instance.FPSGame;
        
        fpsText.text = "FPS: " + fps_slider_values[Settings.Instance.FPSGame];
 
        scrControlsToggle.isOn = Settings.Instance.scrControls; 
        ...
        globalServerToggle.isOn = Settings.Instance.globalServer;
        vibrationToggle.isOn = Settings.Instance.vibration;
        useDpadToggle.isOn = Settings.Instance.useDpad;

        QualitySettings.vSyncCount = Settings.Instance.vsync ? 1 : 0;
    }

    private float selection_time = 5f;
    ...
    void Update() {
        ...
        joinRoomTxt.text = selectedRoomIcon == null ? "Join Random Lobby" : "Join Selected Lobby";
        ...
        //setting strings
        fpsText.text = "FPS: " + fps_slider_values[Settings.Instance.FPSGame];
        lobbyServerText.text = Settings.Instance.globalServer ? "Global Server" : "Mobile Server";
    }
    ...
    public void ChangeLevel(int index) {
        ...
        LocalChatMessage("MvsLO:\n" + "Map set to: " + levelDropdown.options[index].text, Color.red);
    ...
    private void CheckAndJoinRandomRoom() {
        ...
        foreach (RoomInfo room in availableRooms) {
            if (room.CustomProperties.ContainsKey(Enums.NetRoomProperties.GameStarted) &&
                !(bool)room.CustomProperties[Enums.NetRoomProperties.GameStarted]) {
                validRooms.Add(room);
            }
        }

        if (validRooms.Count > 0) {
            RoomInfo randomRoom = validRooms[Random.Range(0, validRooms.Count)];
            PhotonNetwork.JoinRoom(randomRoom.Name);
        } else {
            OpenErrorBox("No available rooms that are not in progress.");
            Debug.Log("No available rooms that are not in progress.");
        }
    }
    ...
    public void Kick(Player target) {
        if (target.IsLocal) {
            LocalChatMessage("MvsLO:\n" + "While you can kick yourself, it's probably not what you meant to do.", Color.red);
            return;
        }
        PhotonNetwork.CloseConnection(target);
        LocalChatMessage("MvsLO:\n" + $"Successfully kicked {target.GetUniqueNickname()}", Color.red);
    }

    public void Promote(Player target) {
        if (target.IsLocal) {
            LocalChatMessage("MvsLO:\n" + "You are already the host..?", Color.red);
            return;
        }
        PhotonNetwork.SetMasterClient(target);
        LocalChatMessage("MvsLO:\n" + $"Promoted {target.GetUniqueNickname()} to be the host", Color.red);
    }

    public void Mute(Player target) {
        if (target.IsLocal) {
            LocalChatMessage("MvsLO:\n" + "While you can mute yourself, it's probably not what you meant to do.", Color.red);
            return;
        }
        Utils.GetCustomProperty(Enums.NetRoomProperties.Mutes, out object[] mutes);
        List<object> mutesList = new(mutes);
        if (mutes.Contains(target.UserId)) {
            LocalChatMessage("MvsLO:\n" + $"Successfully unmuted {target.GetUniqueNickname()}", Color.red);
            mutesList.Remove(target.UserId);
        } else {
            LocalChatMessage("MvsLO:\n" + $"Successfully muted {target.GetUniqueNickname()}", Color.red);
    ...
    public void BanOrUnban(string playername) {
        ...
        LocalChatMessage("MvsLO:\n" + $"Error: Unknown player {playername}", Color.red);
    }

    public void Ban(Player target) {
        if (target.IsLocal) {
            LocalChatMessage("MvsLO:\n" + "While you can ban yourself, it's probably not what you meant to do.", Color.red);
            return;
        }
        ...
        PhotonNetwork.CloseConnection(target);
        LocalChatMessage("MvsLO:\n" + $"Successfully banned {target.GetUniqueNickname()}", Color.red);
    }
    ...
    private void Unban(NameIdPair targetPair) {
        ...
        PhotonNetwork.CurrentRoom.SetCustomProperties(table, null, NetworkUtils.forward);
        LocalChatMessage("MvsLO:\n" + $"Successfully unbanned {targetPair.name}", Color.red);
    }

    private void RunCommand(string[] args) {
        if (!PhotonNetwork.IsMasterClient) {
            LocalChatMessage("MvsLO:\n" + "You cannot use room commands if you aren't the host!", Color.red);
            return;
        }
        ...
        case "kick": {
            if (args.Length < 2) {
                LocalChatMessage("MvsLO:\n" + "Usage: /kick <player name>", Color.red);
                return;
            }
            ...
            if (target == null) {
                LocalChatMessage("MvsLO:\n" + $"Error: Unknown player {args[1]}", Color.red);
                return;
            }
            Kick(target);
            return;
        ...
        case "host": {
            if (args.Length < 2) {
                LocalChatMessage("MvsLO:\n" + "Usage: /host <player name>", Color.red);
                return;
            }
            string strTarget = args[1].ToLower();
            Player target = PhotonNetwork.CurrentRoom.Players.Values.FirstOrDefault(pl => pl.GetUniqueNickname().ToLower() == strTarget);
        ...
        */
        ...
        case "ban": {
            if (args.Length < 2) {
                LocalChatMessage("MvsLO:\n" + "Usage: /ban <player name>", Color.red);
                return;
            }
            BanOrUnban(args[1]);
            return;
        ...
        LocalChatMessage("MvsLO:\n" + $"Error: Unknown command. Try /help for help.", Color.red);
        return;
    }
