...
public class PlayerController : MonoBehaviourPun, IFreezableEntity, ICustomSerializeView, IOnPhotonViewPreNetDestroy {
    ...
    void do_vibrate(long ms){
        if (GlobalController.Instance.settings.vibration){
            GlobalController.Instance.vibrationScript.Vibrate(ms);
            Debug.Log("Vibrating for " + ms + "ms");
        }

        //bump : 100 ms
        //stomp : 200 ms
        //stomp while mini: 150ms
        //groundpound/spin : 300ms
        //giant bump: 350ms
        //giant stomp: 400ms
    }
        ...
        case "Player": {
            ...
            foreach (ContactPoint2D contact in contacts) {
                ...
                if (state == Enums.PowerupState.BlueShell && otherAbove && (!other.groundpound && !other.drill) && (crouching || groundpound)) {
                    body.velocity = new Vector2(SPEED_STAGE_MAX[RUN_STAGE] * 0.9f * (otherObj.transform.position.x < body.position.x ? 1 : -1), body.velocity.y);
                }
        ...
        case "Fireball": {
            ...
            if (!fireball.isIceball) {
                ...
                if (!Frozen && !frozenObject && !pipeEntering) {
                    ...
                    do_vibrate(200);  // Ice fireball hit
                    return;
                }
            }
            break;
    ...
    public void AttemptCollectPowerup(int powerupID, PhotonMessageInfo info) {
        ...
        //if (current_powerup_power > powerup.powerupScriptable.power_value)
            //return;
        //we dont let the user grab a powerup if they are doing any of the following actions
        /*if (inShell || usedPropellerThisJump || flying )
            return;*/
    ...
    private int current_powerup_power = 0;
    ...
    protected void Powerup(int actor, PhotonMessageInfo info) {
        ...
        /*if (current_powerup_power > powerup.power_value){
            return;
        }*/
        ...
        if (view.IsMine)
            PhotonNetwork.Destroy(view);
            current_powerup_power = (int)powerup.power_value;
        Destroy(view.gameObject);
    ...
    void ThrowHeldItem(bool left, bool right, bool crouch) {
        Debug.Log($"ThrowHeldItem called. throw_holded_item before: {throw_holded_item}");

        if (throw_holded_item && !holding)
            throw_holded_item = false;

        if (!((/*!functionallyRunning || */throw_holded_item || state == Enums.PowerupState.MiniMushroom || state == Enums.PowerupState.MegaMushroom || invincible > 0 || flying || propeller) && holding))
            return;
        ...
        Debug.Log($"ThrowHeldItem executed. throw_holded_item after: {throw_holded_item}");
    }
